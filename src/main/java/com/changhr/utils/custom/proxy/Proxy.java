...
public class Proxy {
    ...
    public static Object newProxyInstance(Class intfc, InvocationHandler handler) throws Exception {
        ...
        JavaFile javaFile = JavaFile
                .builder("com.changhr.utils.custom.proxy", typeSpecBuilder.build())
                .build();
        ...
        // 编译
        JavaCompiler.compile(new File(sourcePath + "/com/changhr/utils/custom/proxy/DynamicProxy.java"));

        // 使用反射 load 到内存
        URL[] urls = {new URL("file:/" + sourcePath)};
        URLClassLoader classLoader = new URLClassLoader(urls);
        Class<?> clazz = classLoader.loadClass("com.changhr.utils.custom.proxy.DynamicProxy");
